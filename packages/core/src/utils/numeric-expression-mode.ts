...
export function buildNumericExpressionGuidanceForBook(
  book: BookNumericExpressionInput,
  genreProfile: Pick<GenreProfile, "id" | "name" | "numericalSystem"> | null | undefined,
  language: "zh" | "en",
...
export function buildNumericExpressionGuidance(
  ...
  if (language === "en") {
    return buildEnglishGuidance(resolution, audience);
  }
  return buildChineseGuidance(resolution, audience);
}
...
- 可以保留自然数字：三息、一炷香、七副玉棺、千年、半尺、聚气九层、化灵门槛、第七容器。
