...
5. 纠偏铁律（防止思维惯性）：当本章正文或观察日志纠正了某项关键指标、资金余额、物品归属等核心状态时，你必须在更新后的状态卡（RUNTIME_STATE_DELTA）中物理修改对应的值。绝对禁止在输出的更新状态中残留、照抄与本章最新事实相违背的旧状态陈述（例如，如果正文已改写为没动用本金、用营业额付账，则输出的更新状态中绝对不能出现“本金减少”或“余额剩下994”等旧版脏数据）。
