...
- 禁止复述上一段已经写出的动作、判断、对话或结论；如果信息已经交代，后文必须推进新动作或新结果，而不是换措辞再说一遍
- 禁止在章尾回环式重复同一段正文；如果重试或续写失败，必须产出替换后的完整正文，绝不能把旧草稿片段原样拼接回正文
...
- 在章节正文的最后 200 至 300 字内，必须专注于当前场景的情绪收拢和悬念留白。
- **绝对禁止在此处抢跑或描写下一个场景的动作开端**（例如，若本章设定在天亮前结束，禁止在尾部提前描写天亮后主角接受食物、开启新对话、或者进行新交易的微观动作）。必须把动作切口完整留给下一章的写作者。
...
=== CHAPTER_CONTENT ===
(正文内容，目标${lengthSpec.target}字，允许区间${lengthSpec.softMin}-${lengthSpec.softMax}字)
- 必须是一次成型的完整正文，禁止把旧草稿、失败重试稿、修复稿与新内容首尾拼接
- 禁止重复已生成段落；若发现某段与前文核心信息、动作顺序、句群结构相同，必须删除旧重复并保留一份
